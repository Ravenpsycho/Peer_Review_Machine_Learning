...
Model stacking as covered in the lessons cannot be performed (too high correlation between methods) increase accuracy by stacking another model to see if the tradeoff (interpretability) is worth the boost.
...
```{r}
# Agreement accuracy
agr.acc <- sum(predict.gbm.fac == predict(mod.rf))/length(training$classe)
...
stack.df <- data.frame(pred.rf = predict(mod.rf, newdata = validation),
                       pred.gbm = predict.gbm.fac,
...
print(accuracy.stack); print(table.stack)
```
